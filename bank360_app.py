...
class ComplaintAnalyzer:
    ...
    def similar_complaints(self, query: str, complaint_texts: List[str], top_k: int = 5) -> List[Dict]:
        """Find similar complaints using TF-IDF similarity"""
        if not complaint_texts:
            return []
        
        try:
            # Create corpus with query and complaint texts
            corpus = [query] + complaint_texts
            
            # Vectorize
            vectorizer = TfidfVectorizer(stop_words=['və', 'bu', 'olan', 'the', 'and', 'or'])
            tfidf_matrix = vectorizer.fit_transform(corpus)
            
            # Calculate similarities with query (first document)
...
def product_insights_page(gemini_api: GeminiAPI):
    ...
    if selected_customer:
        ...
        for _, row in top_products.iterrows():
            st.write(f"• {row['Product'].replace('_', ' ').title()}: {row['Propensity']:.1%} likelihood")
